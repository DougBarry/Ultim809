...
# December 15, 2010
#
# Modifications for generic ROM burning by Doug Barry 20160710
# Also upgraded to Python 3.5 syntax.
#
# Usage:
...
import getopt, sys, serial, struct
...
def writeMem(ser, addr, chunk):
    ...
    packet.extend(FN_WRITE_MEM)
    packet.extend(struct.pack("H", addrhi))
    packet.extend(struct.pack("H", addrlo))
    packet.extend(struct.pack("I", numbytes))
...
def readMem(ser, addr, numbytes):
    ...
    print("waiting for reply []", numbytes)
...
# --------------------
def usage():
    """usage: ser09 [-p device] <action> [file.s19]
